...
- \usepackage{fancyhdr}
- \pagestyle{fancy}
- \fancyhead[RE,RO]{CYO proyect - Heart Disease Prediction}
- \fancyfoot[RE,LO]{Kiko Núñez}
- \fancyfoot[LE,RO]{January 2020}
output:
...
```{r load_and_tidy, echo = FALSE, message = FALSE}
...
df2 <- df %>%
  filter(
    thal != 0 & ca != 4 # remove values corresponding to NA in original dataset
  ) %>%
...
kable(head(df2[,8:14]), caption = "Head of the dataset. Columns 8 to 14.")
...
```{r gender visual continuous, echo = FALSE, warning = FALSE}
...
res <- ggarrange(a1, b1, c1, d1, e1, f1, 
...
```{r gender visual categorical, echo = FALSE, warning = FALSE}
# Disease status
...
pdf(NULL)
res <- ggarrange(g1, h1, i1, j1, k1, l1, m1, n1, 
...
```{r only males continuous, echo = FALSE}
...
pdf(NULL)
res <- ggarrange(a2, b2, c2, d2, e2, f2, 
          ncol = 2, nrow = 3, common.legend = TRUE,
            align = "hv")
...
```{r only males categorical, echo = FALSE}
# Disease status
...
pdf(NULL)
res <- ggarrange(g2, h2, i2, j2, k2, l2, m2, n2, 
          ncol = 2, nrow = 4, common.legend = TRUE,
          align = "hv")
x <- dev.off()
suppressWarnings(res)
...
```{r only females continuous, echo = FALSE, warning = FALSE}
...
```{r only females categorical, echo = FALSE, warning = FALSE}
# Disease status
g2 <- ggplot(df, aes(x = target, fill = target)) +
  geom_bar(width = 0.5, position = 'dodge') + 
  labs(x = "", y = "", title = "Target") +
...
```{r disease and gender summary, echo = FALSE}
knitr::kable(df2 %>%
  group_by(target, sex) %>%
  summarise(
    n = n(),
...
``` {r splits, warning = FALSE, echo = FALSE}
